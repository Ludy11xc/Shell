 ...
 */
// const size_t INITIAL_CAPACITY = 8; ALREADY DEFINED IN HEADER
/**
 * 'GROWTH_FACTOR' is how much the vector will grow by in automatic reallocation
 * (2 means double).
 */
// const size_t GROWTH_FACTOR = 2; ALREADY DEFINED IN HEADER
...
void vector_resize(vector *this, size_t n) {
    ...
    if (n < vector_size(this)) {
        for (size_t i = n; i < vector_size(this); i++) {
            if (this->destructor != NULL) this->destructor(this->array[i]);
            else free(this->array[i]);
        }
        this->size = n;
...
void vector_push_back(vector *this, void *element) {
    ...
    if (this->copy_constructor != NULL) {
        ...
        this->array[vector_size(this)] = element;
    }
    this->size++;
}
...
void vector_pop_back(vector *this) {
    ...
    if (this->destructor != NULL) {
        ...
        free(this->array[vector_size(this) - 1]);
    }
    this->size--;
}
...
void vector_insert(vector *this, size_t position, void *element) {
    ...
    void *prev;
    if (this->copy_constructor != NULL) {
        prev = this->copy_constructor(element);
    } else {
        prev = element;
    }
...
void vector_erase(vector *this, size_t position) {
    ...
    // your code here
    if (this->destructor != NULL) this->destructor(this->array[position]);
    //else free(this->array[position]);
    for (size_t i = position + 1; i < vector_size(this); i++) {
